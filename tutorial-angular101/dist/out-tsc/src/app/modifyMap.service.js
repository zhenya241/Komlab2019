...
var ModifyMapService = /** @class */ (function () {
    ...
    ModifyMapService.prototype.wrap = function (text, width) {
        text.each(function () {
            // let r = 0;
            var text = d3.select(this), words = text.text().split('').reverse(), word, line = [], lineNumber = 0, 
    ...
    ModifyMapService.prototype.buildMicroMap = function (svg, path, links, glossary, glossaries, gText, gTexts, gImage, circle, nodes, linkword, linkwords, nodesNextMap, circleNextMap, offset) {
        ...
        d3.select('svg').attr('nodeTotalNumber', function (d) {
            return nodes.length;
        });
        ...
        g.append('svg:ellipse')
            ...
            d3.select('svg')
                .append('svg:path')
                .attr('class', 'dragLine')
                .attr('sourceX', function (d) { return d3.event.x; })
                .attr('sourceY', function (d) { return d3.event.y; })
                .attr('targetX', function (d) { return d3.event.x; })
                .attr('targetY', function (d) { return d3.event.y; })
                .attr('d', function (d) {
                // calculate the d attribute for path
            ...
            d3.select('path.dragLine')
                .attr('sourceX', function (d) { return d3.select(_this).attr('cx'); })
                .attr('sourceY', function (d) { return d3.select(_this).attr('cy'); })
                .attr('targetX', function (d) { return d3.event.x; })
                .attr('targetY', function (d) { return d3.event.y; })
                .attr('d', function (d) {
                // calculate the d attribute for path
                return "M" + d3.select(_this).attr('cx') + "," + d3.select(_this).attr('cy') + "L" + d3.event.x + "," + d3.event.y;
            })
                // set arrow style
            ...
            if (targetNodeId === undefined) {
                ...
                d3.select('svg')
                    ...
                    .attr('d', function (d) {
                    // calculate the d attribute for path
                    return "M" + d3.select(_this).attr('cx') + "," + d3.select(_this).attr('cy') + "L" + endLinkword['x'] + "," + endLinkword['y'];
                })
                    // set arrow style
                    .style('marker-end', 'url(#end-arrow)');
            }
                ...
                d3.select('svg')
                    ...
                    .attr('d', function (d) {
                    // calculate the d attribute for path
                    return "M" + d3.select(_this).attr('cx') + "," + d3.select(_this).attr('cy') + "L" + endNode['x'] + "," + endNode['y'];
                })
                    // set arrow style
                    .style('marker-end', 'url(#end-arrow)');
            }
        ...
        d3.select('svg').selectAll('ellipse.node').on('click', function (d) {
            var id = d['id'];
        ...
        d3.select('svg').selectAll('text.gText').attr('visibility', 'hidden');
        d3.select('svg').selectAll('image.gImage').attr('visibility', 'hidden');
        d3.select('svg').selectAll('ellipse.node').style('fill', 'green').on('mousedown', function (d) {
            if (d3.event.button === 0) {
                var id = d['id'];
                if (d3.select('svg').selectAll('rect.gRect').filter(function (a, i) {
                    return i === id;
...
export { ModifyMapService };
//# sourceMappingURL=modifyMap.service.js.map