...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() { // check the url and open corresponding function
            switch (true) {
        ...
        function getUsers() { // Just for debugging
            // if not registered and logged in, limit its routes
            if (!isLoggedIn()) return unauthorized();
            return ok(users);
        }

        function deleteUser() { // Just for testing and debugging
        ...
        function idFromUrl() { // Takes user id from the url
